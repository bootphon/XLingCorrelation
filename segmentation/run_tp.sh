...
for line in $(find $1 -name '*prepared_syllable*'); do
	...
	number=$(echo $(basename $line) | egrep -o '[[:digit:]]' | head -n1)
        echo $line $(dirname $line)/gold$number\.txt
        ./tp.sh $line 'syllable' $(dirname $line)/gold$number\.txt relative forward
        ./tp.sh $line 'syllable' $(dirname $line)/gold$number\.txt relative backward
        ./tp.sh $line 'syllable' $(dirname $line)/gold$number\.txt absolute forward
        ./tp.sh $line 'syllable' $(dirname $line)/gold$number\.txt absolute backward
done
